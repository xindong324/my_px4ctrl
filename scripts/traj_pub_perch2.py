...
start_state = np.array([0,0,1.0])
end_state = np.array([5,0.15,2.5])
end2_state = np.array([10,0,1.0])

sv_state = np.array([0, 0, 0])

ev_state = np.array([1, 0, 0])
ev2_state = np.array([0, 0, 0])
...
# start_state = np.array([0,0,1.0])
# end_state = np.array([2,0,0.5])

# pos_min_constrain = np.array([-1, -1, 0])
# pos_max_constrain = np.array([3, 2, 2.5])
...
bx2 =[end_state[0], end2_state[0], ev_state[0], ev2_state[0], ea_state[0], ea2_state[0]]
by2 =[end_state[1], end2_state[1], ev_state[1], ev2_state[1], ea_state[1], ea2_state[1]]
bz2 =[end_state[2], end2_state[2], ev_state[2], ev2_state[2], ea_state[2], ea2_state[2]]

coeff_x2 = np.linalg.solve(A,bx2)
coeff_y2 = np.linalg.solve(A,by2)
coeff_z2 = np.linalg.solve(A,bz2)
coeff2 = np.array([coeff_x2, coeff_y2, coeff_z2])
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        t_vec = np.array([[t2**5, t2**4, t2**3, t2**2, t2, 1],
        ...
        pos_vel_acc = np.dot(coeff_r , t_vec)
        future_pos = np.dot(coeff_r, t_fu_vec.T)
